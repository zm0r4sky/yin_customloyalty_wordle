...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Nickname elements
    const inputNickname = document.getElementById('input-nickname') as HTMLInputElement;
    const btnSaveNickname = document.getElementById('btn-save-nickname') as HTMLButtonElement;
        ...
        if (response.guesses && response.guesses.length > 0) {
            for (let i = 0; i < response.guesses.length; i++) {
                ...
                for (let c = 0; c < wordLength; c++) {
                    const tile = document.getElementById(`tile-${currentRow}-${c}`) as HTMLElement;
                    if (tile) {
    ...
    skipAdBtn.addEventListener('click', async () => {
        try {
            ...
            if (currentGameType === 'free') {
                if (endDailyStats) endDailyStats.style.display = 'none';
                if (endFreeStats) endFreeStats.style.display = 'block';
                ...
                if (statFreePlayed) statFreePlayed.textContent = played.toString();
                if (statFreeWinRatio) statFreeWinRatio.textContent = ratio + "%";
            } else {
                if (endDailyStats) endDailyStats.style.display = 'block';
                if (endFreeStats) endFreeStats.style.display = 'none';
